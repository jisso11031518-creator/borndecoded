...
function FourPillarsPage({ engineData }) {
  ...
  function PillarCell({ label, pillar }) {
    if (!pillar) return el(View, { style: { flex: 1, alignItems: 'center', padding: 4 } },
      el(Text, { style: { fontFamily: 'Body', fontSize: 9, color: MUTED } }, label),
      el(Text, { style: { fontFamily: 'Body', fontSize: 10, color: MUTED, marginTop: 8 } }, 'Unknown'),
    );
    const tgColor = ELEM_COLOR[pillar.tiangan?.elementEn] || MUTED;
    const dzColor = ELEM_COLOR[pillar.dizhi?.elementEn] || MUTED;
    return el(View, { style: { flex: 1, alignItems: 'center', padding: 4 } },
      el(Text, { style: { fontFamily: 'BodyBold', fontSize: 9, color: GOLD, marginBottom: 6 } }, label),
      // Tiangan
      el(View, { style: { backgroundColor: tgColor, borderRadius: 4, padding: '4 8', marginBottom: 3, minWidth: 70, alignItems: 'center' } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 14, color: '#fff' } }, pillar.tiangan?.hanja || ''),
      ),
      el(Text, { style: { fontFamily: 'Body', fontSize: 7.5, color: BROWN, textAlign: 'center', marginBottom: 6 } },
        `${pillar.romanization?.split('')[0] || ''} (${pillar.tiangan?.yinYangEn || ''} ${pillar.tiangan?.elementEn || ''})`
      ),
      // Dizhi
      el(View, { style: { backgroundColor: dzColor, borderRadius: 4, padding: '4 8', marginBottom: 3, minWidth: 70, alignItems: 'center' } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 14, color: '#fff' } }, pillar.dizhi?.hanja || ''),
      ),
      el(Text, { style: { fontFamily: 'Body', fontSize: 7.5, color: BROWN, textAlign: 'center' } },
        `${pillar.dizhi?.animal || ''} (${pillar.dizhi?.elementEn || ''})`
      ),
    );
  }
  ...
  function ElemBar({ element, percent, missing }) {
    ...
    const barWidth = Math.max(2, (percent / 100) * 200);
    return el(View, { style: { flexDirection: 'row', alignItems: 'center', marginBottom: 6 } },
      el(Text, { style: { fontFamily: 'Body', fontSize: 9, color: BROWN, width: 45 } }, element),
      el(View, { style: { width: 210, height: 14, backgroundColor: '#E8E0D6', borderRadius: 3, overflow: 'hidden' } },
        el(View, { style: { width: barWidth, height: 14, backgroundColor: color, borderRadius: 3 } }),
      ),
      el(Text, { style: { fontFamily: 'Body', fontSize: 9, color: BROWN, marginLeft: 6, width: 55 } },
  ...
  const dmHanja = fp?.day?.tiangan?.hanja || '';
  ...
  return el(Page, { size: [PW, PH], style: s.pageBody },
    el(View, { style: { flex: 1, justifyContent: 'center' } },
      ...
      el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.3, marginBottom: 16 } }),

      // 4 Pillars Table
      el(View, { style: { flexDirection: 'row', marginBottom: 20, justifyContent: 'center' } },
      ...
      // Divider
      el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.2, marginBottom: 16 } }),

      // Element Distribution
      el(Text, { style: { fontFamily: 'BodyBold', fontSize: 11, color: BROWN, marginBottom: 8 } }, 'Element Distribution'),
      ...['Wood', 'Fire', 'Earth', 'Metal', 'Water'].map(elem => {
        const data = elDist?.english?.[elem];
        const pct = data?.percent || 0;
        const isMissing = elDist?.missing?.includes({ Wood: '목', Fire: '화', Earth: '토', Metal: '금', Water: '수' }[elem]);
        return el(ElemBar, { key: elem, element: elem, percent: pct, missing: isMissing });
      }),

      // Divider
      el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.2, marginTop: 12, marginBottom: 16 } }),

      // Day Master Highlight
      el(View, { style: { alignItems: 'center', backgroundColor: '#F0E8DD', borderRadius: 8, padding: 14 } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 9, color: GOLD, letterSpacing: 2, marginBottom: 6 } }, 'YOUR DAY MASTER'),
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 22, color: ELEM_COLOR[dmElement] || BROWN, marginBottom: 4 } }, dmHanja),
        el(Text, { style: { fontFamily: 'Body', fontSize: 12, color: BROWN, marginBottom: 4 } }, `${dmYinYang} ${dmElement} — ${dmStrength}`),
        dayStage && el(Text, { style: { fontFamily: 'Body', fontSize: 10, color: MUTED } },
          `${dayStage.english} Stage (${dayStage.korean})`
        ),
      ),
    ),
  );
