...
function FourPillarsPage({ engineData }) {
  ...
  // Pillar cell helper — fixed height rows for consistent alignment
  const CELL_W = 80;
  const BOX_H = 28;
  function PillarCell({ label, pillar }) {
    if (!pillar) return el(View, { style: { width: CELL_W, alignItems: 'center', paddingHorizontal: 2 } },
      el(Text, { style: { fontFamily: 'BodyBold', fontSize: 9, color: GOLD, height: 18, marginBottom: 4 } }, label),
      el(View, { style: { height: BOX_H, justifyContent: 'center', alignItems: 'center' } },
        el(Text, { style: { fontFamily: 'Body', fontSize: 10, color: MUTED } }, 'Unknown'),
      ),
    );
    const tgColor = ELEM_COLOR[pillar.tiangan?.elementEn] || MUTED;
    const dzColor = ELEM_COLOR[pillar.dizhi?.elementEn] || MUTED;
    return el(View, { style: { width: CELL_W, alignItems: 'center', paddingHorizontal: 2 } },
      // Row 1: Label
      el(Text, { style: { fontFamily: 'BodyBold', fontSize: 9, color: GOLD, height: 18, marginBottom: 4 } }, label),
      // Row 2: Tiangan box
      el(View, { style: { backgroundColor: tgColor, borderRadius: 4, height: BOX_H, width: 76, justifyContent: 'center', alignItems: 'center', marginBottom: 2 } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 10, color: '#fff' } },
          `${pillar.tiangan?.yinYangEn || ''} ${pillar.tiangan?.elementEn || ''}`
        ),
      ),
      // Row 3: Tiangan label
      el(Text, { style: { fontFamily: 'Body', fontSize: 7, color: MUTED, height: 12, textAlign: 'center' } }, 'Stem'),
      // Row 4: Dizhi box
      el(View, { style: { backgroundColor: dzColor, borderRadius: 4, height: BOX_H, width: 76, justifyContent: 'center', alignItems: 'center', marginBottom: 2 } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 10, color: '#fff' } }, pillar.dizhi?.animal || ''),
      ),
      // Row 5: Dizhi element label
      el(Text, { style: { fontFamily: 'Body', fontSize: 7, color: MUTED, height: 12, textAlign: 'center' } },
