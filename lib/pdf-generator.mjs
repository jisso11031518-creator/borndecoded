...
function clean(text) {
  ...
  text = text.replace(/The\s*-\s*and\s*-\s*stem combinations/gi, 'The stem combinations'); // empty stem name placeholders
...
function SajuCard({ name, dayMaster, metaphor, coverImage }) {
  return el(Page, { size: [PW, PH], style: { ...s.pageBody, justifyContent: 'center', alignItems: 'center' } },
    coverImage && el(Image, { src: coverImage, style: { width: 140, height: 198, marginBottom: 20, borderRadius: 8, objectFit: 'contain' } }),
...
function CompatibilityCard({ person1Name, person2Name, score, gradeName, metaphor, coverImage }) {
  return el(Page, { size: [PW, PH], style: { ...s.pageBody, justifyContent: 'center', alignItems: 'center' } },
    coverImage && el(Image, { src: coverImage, style: { width: 140, height: 198, marginBottom: 20, borderRadius: 8, objectFit: 'contain' } }),
    el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.4, width: 160, marginBottom: 18 } }),
    el(Text, { style: { fontFamily: 'BodyBold', fontSize: 18, color: BROWN, textAlign: 'center', marginBottom: 4 } },
      `${clean(person1Name)} `,
      el(Text, { style: { fontFamily: 'Symbol', color: GOLD } }, '\u2726'),
      ` ${clean(person2Name)}`,
    ),
...
function FourPillarsPage({ engineData }) {
  ...
  function ElemBar({ element, percent, missing }) {
    ...
    return el(View, { style: { flexDirection: 'row', alignItems: 'center', marginBottom: 5 } },
      el(Text, { style: { fontFamily: 'Body', fontSize: 8, color: BROWN, width: 44 } }, element),
      el(View, { style: { width: 190, height: 10, backgroundColor: '#EDE6DC', borderRadius: 5, overflow: 'hidden' } },
        el(View, { style: { width: barWidth, height: 10, backgroundColor: color, borderRadius: 5, opacity: 0.8 } }),
      ),
      el(Text, { style: { fontFamily: 'Body', fontSize: 7.5, color: missing ? '#C75B3F' : MUTED, marginLeft: 6, width: 55 } },
  ...
  return el(Page, { size: [PW, PH], style: s.pageBody },
    el(View, { style: { flex: 1, justifyContent: 'center' } },
      // Title
      el(Text, { style: { fontFamily: 'Script', fontSize: 22, color: GOLD, textAlign: 'center', marginBottom: 4 } }, 'Your Four Pillars at a Glance'),
      el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.3, width: 200, alignSelf: 'center', marginBottom: 16 } }),

      // 4 Pillars
      el(View, { style: { flexDirection: 'row', marginBottom: 16, justifyContent: 'center', gap: 6 } },
      ...
      // Divider
      el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.15, width: 240, alignSelf: 'center', marginBottom: 14 } }),

      // Element Distribution
      el(Text, { style: { fontFamily: 'Script', fontSize: 16, color: GOLD, textAlign: 'center', marginBottom: 8 } }, 'Element Distribution'),
      ...
      // Divider
      el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.15, width: 240, alignSelf: 'center', marginTop: 10, marginBottom: 14 } }),

      // Day Master — gold-framed card
      el(View, { style: { alignItems: 'center', borderWidth: 1, borderColor: GOLD, borderRadius: 12, padding: 14, marginHorizontal: 40 } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 8, color: GOLD, letterSpacing: 2.5, marginBottom: 6 } }, 'YOUR DAY MASTER'),
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 20, color: dmColor, marginBottom: 4 } },
          `${dmYinYang} ${dmElement}`
        ),
...
function CompatibilityInfoPage({ engineData }) {
  ...
  // Dual element bar — symmetric layout
  const BAR_W = 80;
  const LBL_W = 30;
  const CTR_W = 46;
  function DualElemBar({ element }) {
    ...
    const p1W = Math.max(2, (p1Pct / 50) * BAR_W);
    const p2W = Math.max(2, (p2Pct / 50) * BAR_W);
    const fills = (p1Miss && p2Pct > 0) || (p2Miss && p1Pct > 0);
    return el(View, { style: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 7 } },
      // P1 label
      el(Text, { style: { fontFamily: 'Body', fontSize: 6, color: p1Miss ? '#C75B3F' : MUTED, width: LBL_W, textAlign: 'right', marginRight: 3 } },
        p1Miss ? 'Missing' : `${p1Pct}%`
      ),
      // P1 bar (right-aligned fill)
      el(View, { style: { width: BAR_W, height: 10, backgroundColor: '#EDE6DC', borderRadius: 5, overflow: 'hidden', flexDirection: 'row', justifyContent: 'flex-end' } },
        !p1Miss && el(View, { style: { width: p1W, height: 10, backgroundColor: color, borderRadius: 5, opacity: 0.8 } }),
      ),
      // Center element label
      el(View, { style: { width: CTR_W, alignItems: 'center' } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 7, color: color } }, element),
        fills && el(Text, { style: { fontFamily: 'Body', fontSize: 5, color: GOLD } }, 'fills gap'),
      ),
      // P2 bar (left-aligned fill)
      el(View, { style: { width: BAR_W, height: 10, backgroundColor: '#EDE6DC', borderRadius: 5, overflow: 'hidden' } },
        !p2Miss && el(View, { style: { width: p2W, height: 10, backgroundColor: color, borderRadius: 5, opacity: 0.8 } }),
      ),
      // P2 label
      el(Text, { style: { fontFamily: 'Body', fontSize: 6, color: p2Miss ? '#C75B3F' : MUTED, width: LBL_W, marginLeft: 3 } },
        p2Miss ? 'Missing' : `${p2Pct}%`
      ),
    );
  }
  ...
  if (stemHops.length > 0) {
    const hopCount = stemHops.reduce((n, f) => n + (f.impact / 5), 0);
    chemItems.push(`${hopCount} stem ${hopCount > 1 ? 'harmonies' : 'harmony'}`);
  }
  ...
  return el(Page, { size: [PW, PH], style: s.pageBody },
    el(View, { style: { flex: 1, justifyContent: 'center' } },
      ...
      el(View, { style: { alignItems: 'center', marginBottom: 24 } },
        ...
        el(View, { style: { flexDirection: 'row', width: SCALE_W, marginTop: 3, justifyContent: 'space-between' } },
          el(Text, { style: { fontFamily: 'Body', fontSize: 5, color: MUTED, width: 50, textAlign: 'center' } }, 'Grounding'),
          el(Text, { style: { fontFamily: 'Body', fontSize: 5, color: MUTED, width: 50, textAlign: 'center' } }, 'Growth'),
          el(Text, { style: { fontFamily: 'Body', fontSize: 5, color: MUTED, width: 50, textAlign: 'center' } }, 'Harmony'),
          el(Text, { style: { fontFamily: 'Body', fontSize: 5, color: MUTED, width: 60, textAlign: 'center' } }, 'Resonance'),
          el(Text, { style: { fontFamily: 'Body', fontSize: 5, color: MUTED, width: 50, textAlign: 'center' } }, 'Cosmic'),
        ),
      ),
      ...
      // 2) Dual Element Comparison — names aligned with bars
      el(View, { style: { flexDirection: 'row', justifyContent: 'center', marginBottom: 8 } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 8, color: BROWN, width: LBL_W + BAR_W + 3, textAlign: 'center' } }, p1Name),
        el(View, { style: { width: CTR_W } }),
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 8, color: BROWN, width: BAR_W + LBL_W + 3, textAlign: 'center' } }, p2Name),
      ),
      ...['Wood', 'Fire', 'Earth', 'Metal', 'Water'].map(elem =>
...
// ---- Share Page: UGC loop — encourage Instagram sharing ----
function SharePage({ isCompatibility }) {
  return el(Page, { size: [PW, PH], style: { ...s.pageBody, justifyContent: 'center', alignItems: 'center' } },
    // Top decorative line
    el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.5, width: 200, marginBottom: 24 } }),

    // Title
    el(Text, { style: { fontFamily: 'BodyBold', fontSize: 18, color: GOLD, textAlign: 'center', letterSpacing: 2, marginBottom: 20 } },
      'SHARE YOUR CHART'
    ),

    // Subtitle
    el(Text, { style: { fontFamily: 'Body', fontSize: 12, color: BROWN, textAlign: 'center', lineHeight: 1.7, paddingHorizontal: 30, marginBottom: 20 } },
      'Your birth chart is uniquely yours.\nBut the conversation doesn\u2019t end here.'
    ),

    // Decorative divider
    el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.25, width: 120, marginBottom: 20 } }),

    // Instructions
    el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: MUTED, textAlign: 'center', lineHeight: 1.8, paddingHorizontal: 35, marginBottom: 8 } },
      'Screenshot your favorite insight from this reading'
    ),
    el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: MUTED, textAlign: 'center', lineHeight: 1.8, paddingHorizontal: 35, marginBottom: 8 } },
      'Share it on Instagram Stories or Posts'
    ),
    el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: MUTED, textAlign: 'center', lineHeight: 1.8, paddingHorizontal: 35, marginBottom: 20 } },
      'Tag @borndecoded'
    ),

    // Feature CTA
    el(View, { style: { backgroundColor: '#F0E8DB', borderRadius: 10, paddingVertical: 14, paddingHorizontal: 24, marginBottom: 20 } },
      el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: BROWN, textAlign: 'center', lineHeight: 1.7 } },
        'We feature the best shares every week!\nYour chart might inspire someone else\nto discover theirs.'
      ),
    ),

    // Compatibility-only: couples challenge
    isCompatibility && el(View, { style: { backgroundColor: '#F0E8DB', borderRadius: 10, paddingVertical: 14, paddingHorizontal: 24, marginBottom: 20 } },
      el(Text, { style: { fontFamily: 'BodyBold', fontSize: 12, color: GOLD, textAlign: 'center', marginBottom: 6 } },
        'COUPLES CHALLENGE'
      ),
      el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: BROWN, textAlign: 'center', lineHeight: 1.7 } },
        'Share your Compatibility Reading together\nand tag @borndecoded \u2014\ncouples who share get featured first!'
      ),
    ),

    // Decorative divider
    el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.25, width: 120, marginBottom: 18 } }),

    // Brand info
    el(Text, { style: { fontFamily: 'BodyBold', fontSize: 12, color: GOLD, textAlign: 'center', marginBottom: 4 } }, '@borndecoded'),
    el(Text, { style: { fontFamily: 'Body', fontSize: 10, color: MUTED, textAlign: 'center' } }, 'borndecoded.com'),
  );
}
...
export async function generateSajuPdf({ report, coverImageBuffer, bodyImageBuffer, name, birthDate, engineData }) {
  ...
  const doc = el(Document, {},
    ...
    // Share page: UGC loop
    el(SharePage, { key: 'share', isCompatibility: false }),
...
export async function generateCompatibilityPdf({ report, coverImageBuffer, bodyImageBuffer, person1Name, person2Name, birthDate, engineData }) {
  ...
  const doc = el(Document, {},
    ...
    // Share page: UGC loop (couples version)
    el(SharePage, { key: 'share', isCompatibility: true }),
