...
function FourPillarsPage({ engineData }) {
  ...
  // Pillar cell helper — romanization only (no CJK font needed)
  function PillarCell({ label, pillar }) {
    ...
    const roman = pillar.romanization || '';
    const tgRoman = roman.length >= 1 ? roman[0].toUpperCase() + roman.slice(1, Math.ceil(roman.length / 2)) : '';
    const dzRoman = pillar.dizhi?.animal || '';
    return el(View, { style: { flex: 1, alignItems: 'center', padding: 4 } },
      ...
      // Tiangan — element color box with romanization
      el(View, { style: { backgroundColor: tgColor, borderRadius: 4, padding: '6 10', marginBottom: 3, minWidth: 74, alignItems: 'center' } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 11, color: '#fff' } },
          `${pillar.tiangan?.yinYangEn || ''} ${pillar.tiangan?.elementEn || ''}`
        ),
      ),
      // Dizhi — element color box with animal
      el(View, { style: { backgroundColor: dzColor, borderRadius: 4, padding: '6 10', marginBottom: 3, minWidth: 74, alignItems: 'center' } },
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 11, color: '#fff' } }, dzRoman),
      ),
      el(Text, { style: { fontFamily: 'Body', fontSize: 7.5, color: MUTED, textAlign: 'center' } },
  ...
  return el(Page, { size: [PW, PH], style: s.pageBody },
    el(View, { style: { flex: 1, justifyContent: 'center' } },
      ...
      // Day Master Highlight — no CJK, romanization only
      el(View, { style: { alignItems: 'center', backgroundColor: '#F0E8DD', borderRadius: 8, padding: 14 } },
        ...
        el(Text, { style: { fontFamily: 'BodyBold', fontSize: 22, color: ELEM_COLOR[dmElement] || BROWN, marginBottom: 4 } },
          `${dmYinYang} ${dmElement}`
        ),
        el(Text, { style: { fontFamily: 'Body', fontSize: 12, color: BROWN, marginBottom: 4 } }, dmStrength),
