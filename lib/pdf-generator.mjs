...
function clean(text) {
  ...
  return text.replace(/[\u3000-\u9FFF\uF900-\uFAFF\uAC00-\uD7AF]/g, '').replace(/[\u2018\u2019\u201C\u201D]/g, '').replace(/\(\s*\)/g, '').replace(/\s{2,}/g, ' ').trim();
}
...
// ---- Survival Guide — rendered as a dedicated final Page ----
function ClosingPage({ data, product }) {
  if (!data) return el(Page, { size: [PW, PH], style: s.pageBody });
  ...
  return el(Page, { size: [PW, PH], style: { ...s.pageBody, justifyContent: 'space-between' } },
    // Top: Survival Guide content
    el(View, {},
    ...
    // Bottom: closing message + profile + CTA + disclaimer pushed to page bottom
    el(View, {},
      data.closingMessage && el(Text, { style: s.closingMsg }, clean(data.closingMessage)),
      el(View, { style: { alignItems: 'center', marginBottom: 10 } },
        el(Image, { src: 'https://borndecoded.vercel.app/images/profile.png', style: { width: 120, height: 120 } }),
      ),
  ...
  const doc = el(Document, {},
    ...
    el(Page, { size: [PW, PH], style: s.pageBody, wrap: true },
      el(View, { style: s.body },
        ...
        // S2: Essence (starts with Birth Chart metaphor, then Essence content)
        el(SectionBlock, {
          key: 's2', isFirst: true,
          title: report.section2_essence?.title || 'Your Essence',
          metaphorText: metaphor,
    ...
    // Final page: Survival Guide + CTA (dedicated page, no overflow)
    el(ClosingPage, { key: 's10', data: report.section10_survivalGuide, product: 'saju' }),
  );
...
export async function generateCompatibilityPdf({ report, coverImageBuffer, bodyImageBuffer, person1Name, person2Name, birthDate }) {
  ...
  const doc = el(Document, {},
    ...
    // Final page: Couple Guide + CTA (dedicated page)
    el(ClosingPage, { key: 'c10', data: report.section10_coupleGuide, product: 'compatibility' }),
  );
