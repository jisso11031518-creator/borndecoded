...
const s = StyleSheet.create({
  ...
  // Closing — Noto Serif (not script — avoids ligature drops in long dynamic text)
...
// ---- Cover Page (SINGLE page — image + all text) ----
function CoverPage({ coverImage, name, birthDate, product }) {
  ...
  // Cover uses flex layout: top area (image space), bottom area (text)
  // The background image fills the whole page; text is overlaid at the bottom
  return el(Page, { size: 'A4', style: s.page },
    // Background image (full page)
    coverImage && el(Image, { src: coverImage, style: s.bgImage }),

    // Spacer to push text below the watercolor area (top ~55%)
    el(View, { style: { height: '55%' } }),

    // Text block in the lower portion of the page
    el(View, { style: { paddingHorizontal: 60, textAlign: 'center', alignItems: 'center' } },
      el(Text, { style: s.coverBrand }, 'Born Decoded'),
      el(Text, { style: s.coverTagline }, 'YOUR BIRTH WAS CODED. WE DECODE IT.'),
      el(View, { style: { ...s.coverDivider, width: 200, marginHorizontal: 0 } }),
      el(Text, { style: s.coverProduct }, productLabel),
      el(Text, { style: { height: 16 } }, ''),
