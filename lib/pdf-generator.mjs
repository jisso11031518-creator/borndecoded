...
// ---- Cover (full-bleed image only, no text overlay) ----
...
export async function generateSajuPdf({ report, coverImageBuffer, bodyImageBuffer, name, birthDate }) {
  ...
  const doc = el(Document, {},
    // p.1: Cover (full-bleed image + brand overlay)
    el(CoverPage, { coverImage: coverSrc }),

    // p.2+: Body (continuous flow)
    el(Page, { size: 'A4', style: { ...s.page, backgroundColor: IVORY }, wrap: true },
      el(View, { style: s.body },

        // Intro block: brand + name + date + Day Master
        el(View, { style: { textAlign: 'center', alignItems: 'center', marginBottom: 24 }, wrap: false },
          el(Text, { style: { fontFamily: 'BodyBold', fontSize: 18, color: GOLD, letterSpacing: 2, marginBottom: 3 } }, 'BORN DECODED'),
          el(Text, { style: { fontFamily: 'Body', fontSize: 9, color: GOLD, letterSpacing: 3, marginBottom: 10 } }, 'YOUR BIRTH WAS CODED. WE DECODE IT.'),
          el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.4, width: 160, marginBottom: 10 } }),
          el(Text, { style: { fontFamily: 'Body', fontSize: 12, color: MUTED, marginBottom: 12 } }, 'Personal Saju Reading'),
          el(Text, { style: { fontFamily: 'Script', fontSize: 24, color: BROWN, marginBottom: 3 } }, clean(name)),
          el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: MUTED, marginBottom: 16 } }, clean(birthDate)),
          el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.25, width: 100, marginBottom: 14 } }),
          metaphor && el(Text, { style: { fontFamily: 'Script', fontSize: 16, color: GOLD, textAlign: 'center', marginBottom: 10, paddingHorizontal: 20 } }, clean(metaphor)),
          dayMasterExpl && el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: BROWN, lineHeight: 1.6, textAlign: 'center', paddingHorizontal: 10 } }, clean(dayMasterExpl)),
        ),

        // S2: Essence
        el(SectionBlock, {
          key: 's2', isFirst: false,
    ...
    el(Page, { size: 'A4', style: { ...s.page, backgroundColor: IVORY }, wrap: true },
      el(View, { style: s.body },
        ...
        el(View, { style: { textAlign: 'center', alignItems: 'center', marginBottom: 24 }, wrap: false },
          ...
          el(Text, { style: { fontFamily: 'Body', fontSize: 9, color: GOLD, letterSpacing: 3, marginBottom: 10 } }, 'YOUR BIRTH WAS CODED. WE DECODE IT.'),
          el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.4, width: 160, marginBottom: 10 } }),
          el(Text, { style: { fontFamily: 'Body', fontSize: 12, color: MUTED, marginBottom: 12 } }, 'Compatibility Reading'),
          el(Text, { style: { fontFamily: 'Script', fontSize: 22, color: BROWN, marginBottom: 3 } }, clean(compatName)),
          el(View, { style: { height: 1, backgroundColor: GOLD, opacity: 0.25, width: 100, marginBottom: 14 } }),
          compatMetaphor && el(Text, { style: { fontFamily: 'Script', fontSize: 16, color: GOLD, textAlign: 'center', marginBottom: 10, paddingHorizontal: 20 } }, clean(compatMetaphor)),
          compatIntro && el(Text, { style: { fontFamily: 'Body', fontSize: 11, color: BROWN, lineHeight: 1.6, textAlign: 'center', paddingHorizontal: 10 } }, clean(compatIntro)),
        ),

        ...sectionMap.map(([key, fallback], i) =>
          el(SectionBlock, {
            key: `c${i + 2}`, isFirst: false,
