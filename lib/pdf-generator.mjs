...
// ---- Safety — dangerous expression patterns ----
...
export function checkSafetyViolations(text) {
...
function FinalPage({ closingMessage }) {
  return el(Page, { size: [PW, PH], style: { ...s.pageBody, justifyContent: 'center', alignItems: 'center' } },
    ...
    el(Text, { style: s.disclaimer },
      'This reading is for entertainment and self-reflection only. It is not a substitute for professional medical, psychological, legal, or financial advice.\nborndecoded.com | borndecoded@gmail.com'
    ),
  );
}
