...
function ClosingPage({ data, product }) {
  ...
  return el(Page, { size: [PW, PH], style: { ...s.pageBody, justifyContent: 'space-between' } },
    ...
    // Bottom: closing message + CTA + disclaimer pushed to page bottom
    el(View, {},
      data.closingMessage && el(Text, { style: s.closingMsg }, clean(data.closingMessage)),
      el(View, { style: s.ctaBox },
        el(Text, { style: s.ctaBoxText }, ctaLabel),
        el(Text, { style: { ...s.ctaBoxText, fontSize: 9, marginTop: 3 } }, ctaUrl),
      ),
