...
Use no more than 4 star names in the ENTIRE report. For each star, describe the BEHAVIOR or FEELING first, then put the star name in parentheses at the END of that sentence. NEVER start a sentence with a star name.
  ...
  ✅ RIGHT: "There's a restless itch in you that's always planning the next escape, the next city, the next reinvention (Yeokma-sal at work)"
If the behavior can be described without naming the star, OMIT the star name entirely.
