...
## ⚠️ CRITICAL RULES — READ BEFORE WRITING ANYTHING ⚠️

**RULE A — STAR NAMES (MAX 4, FEEL FIRST)**
Use no more than 4 star names in the ENTIRE report. For each star, describe the BEHAVIOR or FEELING first, then put the star name in parentheses at the END of that sentence. NEVER start a sentence with a star name.
  ❌ WRONG: "Your Baekho-sal (White Tiger) gives you explosive energy"
  ❌ WRONG: "The Traveling Horse in your chart makes you restless"
  ❌ WRONG: "Your Naechae-dohwa (Unveiled Blossom) draws people to you with raw magnetic quality, but your Goran-gwasuk (Lonely Phoenix) kicks in and you retreat." (two star names, star-led, label-heavy)
  ✅ RIGHT: "When you finally move, it's zero to full power in one breath (Baekho-sal energy)"
  ✅ RIGHT: "People are pulled toward you without knowing why — there's something magnetic about your quiet intensity (Dohwa-sal energy). But the moment they get close, you instinctively pull back, needing distance to process what just happened." (behavior first, ONE star named, second star omitted)
If the behavior can be described without naming the star, OMIT the star name entirely. When two stars interact, name only the dominant one — describe the other purely through behavior.

**RULE B — DAY MASTER IDENTITY LABEL (MANDATORY)**
In the section1_birthChart.dayMasterExplanation, you MUST include exactly this format somewhere in the text:
  "In Saju terms, you're a [Day Master name] — [one punchy personality hook]."
Examples:
  "In Saju terms, you're a Yin Wood — the quiet one who somehow outlasts everyone."
  "In Saju terms, you're a Yang Fire — the bonfire everyone gathers around but nobody controls."
  "In Saju terms, you're a Yin Metal — precision wrapped in silence."
This line must feel quotable and shareable — like a badge of identity.

**RULE C — Q&A MUST BE WHAT-IF SCENARIOS**
The section9_questions MUST use What-If scenario format. If the user provided custom questions, answer those — but frame the answer around "what if" actionable scenarios with specific timing. If no custom questions, use these EXACT defaults:
  Q1: "What if I start something completely new this year?"
  Q2: "What if I need to make a major financial decision?"
  Q3: "What if I'm feeling stuck or burned out?"
Each answer: 80-120 words. Must contain NEW analysis NOT found in the main body. Include specific months or seasons.

**RULE D — ABSOLUTE BANNED PHRASES**
These phrases are FORBIDDEN. Using ANY of them is a critical error:
  "Let's be brutally honest" | "Let's be real" | "Here's where it gets messy"
  "superpower in disguise" | "curse is actually your gift" | "not weakness — it's strength"
  "like moths to flame" | "Avoid get-rich-quick schemes" | "trust the process"
  "the universe has a plan" | "you are enough" | "embrace your authentic self"
  "Stop trying to X and start Y" | "Your survival strategy depends on"
  "It's not a bug, it's a feature" | "everything happens for a reason"
If you catch yourself about to write any of these, STOP and rephrase using this person's specific chart data.

**RULE E — UNIQUE CLOSING LINE**
The closingMessage in section10 must be completely unique to THIS person. It must callback to their specific Day Master metaphor and cover art image. NEVER use "Your birth was coded. Now you hold the key." — that line is reserved for the PDF footer only, not the report text.
...
13. **STAR NAME USAGE — FEEL FIRST, LABEL SECOND**:
   - MAXIMUM 4 star names per report. Choose the most impactful ones.
   - NEVER lead a sentence with a star name. Describe the behavior/feeling FIRST, then put the star name in parentheses at the END.
     WRONG: "Your White Tiger Star makes you capable of sudden action"
     RIGHT: "When you finally move, it's explosive — zero to full power in seconds (Baekho-sal energy)"
   - If a star name is not essential to the sentence's meaning, omit it entirely.
   - Prioritize: Baekho-sal (action), Dohwa-sal OR Hongnyeom-sal (romance, pick ONE), Yeokma-sal (career/travel), Hwagae-sal (creativity), Hyeonchim-sal (precision), Goran-sal (solitude).
